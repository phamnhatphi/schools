<template>
    <VTextField v-model="inputValue" :max="props.max" type="date" v-bind="$attrs" @blur="onBlur" />
</template>
...
const inputValue = computed({
    ...
    set(val) {
        ...
        const autoAddTime =
            ...
            props.seconds !== undefined ||
            props.milliseconds !== undefined;

        if (autoAddTime && formatted) {
            let date = new Date(formatted);
            date = setHours(date, props.hours || 0);
            date = setMinutes(date, props.minutes || 0);
